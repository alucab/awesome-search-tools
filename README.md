...
* [ReadStat](https://github.com/WizardMac/ReadStat) **—** ReadStat is a MIT licensed command-line tool and C library for reading and converting files from SAS(SAS7BDAT,XPORT), SPSS(POR,SAV,ZSAV), Stata(DTA). The files can be converted to CSV and XLS. There are bindings for Julia, Perl, Python, R and a dockerized version.
